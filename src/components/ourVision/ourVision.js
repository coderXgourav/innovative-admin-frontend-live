...
function OurVisionManagement() {
    ...
    useEffect(() => {
        ...
        const fetcher = async () => {
            let tempOurVisionsData = await fetchOurVision();

            if (tempOurVisionsData?.message === "jwt expired") {
                return navigate("/");
            } else {
    ...
    return (
        <>
            ...
            <div className="row">
                {windowWidth > 768 && <Sidebar activeOption="our-vision-management" />}
                <div className="col-md-10 p-4" style={{ height: '90vh', overflowY: 'auto' }}>
                    ...
                    <div className="d-flex row">
                        <div className="col fw-bold">{`Our Visions(${ovs.length})`}</div>
